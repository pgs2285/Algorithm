...
- [4.힙과 우선순위 큐](#4힙과-우선순위-큐)
  ...
  - [4 - 4. A* 구현하기](#4-4-a-알고리즘-구현하기)
- [5.탐색 트리](#5-탐색트리)
  - [5 - 1. 이진탐색 구현하기](#5-1-이진탐색-구현하기)
  - [5 - 2. 이진탐색트리 구현하기](#5-2-이진탐색트리-구현하기)
  - [5 - 3. 레드블랙트리 구현하기](#5-3-레드-블랙-트리-구현하기)
    - [5 - 3 - a. 레드 블랙 트리의 삽입](#5-3-a-레드-블랙-트리의-삽입)
    - [5 - 3 - b. 레드 블랙 트리의 삭제](#5-3-b-레드-블랙-트리의-삭제) 
...
난이도가 조금 있다! 레드블렉트리를 실제 구현하는건 몇번 하진 않을테니 이론을 외우고, 한번 실습느낌으로 만들어보자.(즉 각잡고 개발해야하는 알고리즘이지 코딩테스트 용은 아니다)     
#### 5-3-a 레드 블랙 트리의 삽입
...
아래는 삽입을 구현한 gif이다. 터미널 창 특성상 파란색이 black트리이고, 빨간색이 red트리이다.

<img src="./GitHubImage/Insert.gif" title="px(픽셀) 크기 설정" alt="pq" width="548">   

#### 5-3-b 레드 블랙 트리의 삭제  
위에서 레드 블랙 트리의 5가지 조건에 대해 알아본적이 있는데, 삭제할때 가장 중요하게 봐야하는건 5번(어떤 노드로부터 시작되어 그에 속한 하위 리프 노드에 도달하는 모든 경로에는 모두 같은 개수의 블랙 노드가 있다) 이다.  
- 1. 삭제할 노드가 RED인 경우. -> 그냥 삭제하면 끝난다.
- 2. 삭제할 노드가 BLACK인 경우. -> 이 경우 대게 5번 규칙이 깨지게 된다. 이미 균형이 맞춰져 있던 RED-BLACK 트리에 블랙을 삭제하면 개수가 맞지 않기 때문이다.(더블 블랙 상태라고 한다) 삭제 알고리즘에서는 Double Black상태인 노드의 형제노드가 핵심이다.
...
구현한 레드블랙트리 선언부는 **[여기(RedBlackTree.h)](./Algorithm/SelfModule/RedBlackTree.h)**를 클릭하면 볼 수 있고,  
레드블랙트리 구현부는 **[여기(RedBlackTree.cpp)](/Algorithm/SelfModule/RedBlackTree.cpp)**를 클릭하면 볼 수 있다.  

결과는 아래와 같다.  

![pq](./GitHubImage/redBlackTree.gif "rbt")
